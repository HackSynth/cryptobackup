...
# CryptoBackup Linux 测试脚本
# 使用方法: chmod +x test-linux.sh && ./test-linux.sh
...
print_header() {
    echo -e "${CYAN}==================================================${NC}"
    echo -e "${CYAN}   CryptoBackup Linux 测试脚本 v1.0.0${NC}"
    echo -e "${CYAN}==================================================${NC}"
    echo ""
}
...
main() {
    ...
    if command -v wget &> /dev/null; then
        ...
        print_error "未找到 wget 或 curl，请安装其中一个"
        exit 1
    fi

    chmod +x cryptobackup
    FILE_SIZE=$(du -h cryptobackup | cut -f1)
...
EOF
    ...
    ORIGINAL_HASH=$(sha256sum testfile.txt | cut -d' ' -f1)
    RESTORED_HASH=$(sha256sum restored.txt | cut -d' ' -f1)
    ...
    # 验证 XOR
    XOR_HASH=$(sha256sum restored_xor.txt | cut -d' ' -f1)
